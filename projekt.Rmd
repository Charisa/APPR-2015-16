...
V različnih državah so drugačni standardi, drugačni indeksi življenjsjih potrebščin, drugačni pogoji za življenje, drugačne razmere, različno geografsko okolje ... 


```{r zakljucek1, echo = FALSE}
kable(head(sort_quality_of_life_2015))
```


Glede na dano tabelo, je najvišja kvaliteta življenja v Švici, ki kar za 27 enot vodi za drugouvrščeno Nemčijo. Za podatke, ki jih imamo na voljo, je Slovenija na 25. mestu, kar je kar nekaj mest pred najbližjimi sosedami (Hrvaško, Italijo in Madržarsko).
Zelo zanimivo je raziskovati, kako se življenjski standardi in pogoji ter kvaliteta storitev razlikuje v bližnjih državah. 
Spletna stran _http://www.numbeo.com_ ima tudi možnost izračuna določenih stroškov v posameznih mestih. Za primerjavo sem vzela nekaj dobrin/storitev (poceni kosilo v povprečni restavraciji, 1/3 liter vode, 1 liter kravjega mleka, 1 kg jabolk in banan, 1 kg paradižnika in krompirja, enosmerna vozovnica javnega prevoza, mesečna vozovnica, tarifa za 1 km vožnje s taksijem, 1 liter bencina, osnovne storitve (voda, komunala, elektrika, ogrevanje ...) v povprečnem 85-kvadratnih metrov velikem stanovanju (brez vključene cene najemnine), tarifa za eno minuto klica v omrežju, internet, mesečna karta za fitnes, obisk kina za eno osebo, par kavbojk, poletna obleka, enosobno stanovanje v centru, enosobno stanovanje izven centra).

```{r zakljucek2, echo = FALSE}
kable(head(cost_of_living_cities))
```

Vsota cen vseh dobrin je v različnih državah zelo drugačna. V Švici, ki je država z največjo kvaliteto življenja, se cene vseh prej naštetih dobrin gibljejo med 2.500 in 4.100 €, medtem ko je pri nas v Sloveniji cena med 995 in 1.200 €. 
Na cene teh dobrin vpliva veliko dejavnikov. Vendar pa se vseeno lahko zanesemo na visoko razvitost določenih držav (kar je razvidno v tabeli _human development index_) in visoko kvaliteto življenja ter ugotovimo, v katerih državah je življenje boljše, v katerih manj.

Da pa končam v pozitivnem duhu - Slovenija ima v primerjavi z ostalimi državami visoko kvaliteto življenja, prav tako razvitost (na področju izobrazbe, naravnega okolja, cen, plač). V nobeni državi pa ne bomo našli rožnatega neba. Zato še misel za na konec: __Povsod je lepo, a doma je najlepše__.